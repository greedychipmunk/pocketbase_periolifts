...
class WorkoutService extends BasePocketBaseService {
  ...
  static const String _programsCollection = 'workout_plans';
  ...
  /// Get workout programs (plans) with optional filtering
  ///
  /// Returns Result<List<WorkoutPlan>> with workout plans or error state
  /// Performance: <500ms per operation
  Future<Result<List<WorkoutPlan>>> getPrograms({
    String? searchQuery,
    int page = 1,
    int perPage = 50,
    bool includeUserOnly = false,
    ...
    try {
      ...
      final programs = records.items.map<WorkoutPlan>((RecordModel record) {
  ...
  /// Create a new workout program (plan)
  ///
  /// [program] Workout plan data to create
  ///
  /// Returns Result<WorkoutPlan> with created workout plan or error state
  /// Performance: <500ms per operation
  Future<Result<WorkoutPlan>> createProgram(WorkoutPlan program) async {
    try {
      // Validate program data
      final validation = _validateWorkoutPlan(program);
      if (validation.isError) {
        return Result.error(validation.error!);
      }
      ...
      // Set user_id for the program
      final programData = program.toJson();
      programData['user_id'] = currentUserId;

      final record = await pb.collection(_programsCollection).create(body: programData);
      final createdProgram = WorkoutPlan.fromJson(record.toJson());

      return Result.success(createdProgram);
    } on ClientException catch (e) {
      return Result.error(ErrorHandler.handlePocketBaseError(e));
    } catch (e) {
      return Result.error(ErrorHandler.handlePocketBaseError(e));
  ...
  /// Update an existing workout program (plan)
  ///
  /// [program] Updated workout plan data
  ///
  /// Returns Result<WorkoutPlan> with updated workout plan or error state
  /// Performance: <500ms per operation
  Future<Result<WorkoutPlan>> updateProgram(WorkoutPlan program) async {
    try {
      ...
      final existingResult = await getProgramById(program.id);
      if (existingResult.isError) {
        return Result.error(existingResult.error!);
      }

      final existingProgram = existingResult.data!;
      if (existingProgram.userId != currentUserId) {
        return Result.error(
          AppError.permission(
            message: 'Cannot update program owned by another user',
          ),
        );
      }
      ...
      final record = await pb
          .collection(_programsCollection)
          .update(program.id, body: program.toJson());
      final updatedProgram = WorkoutPlan.fromJson(record.toJson());
  ...
  ///
  /// [id] Workout program ID to retrieve
  ///
  /// Returns Result<WorkoutPlan> with workout program or error state
  /// Performance: <500ms per operation
  Future<Result<WorkoutPlan>> getProgramById(String id) async {
    try {
      if (id.trim().isEmpty) {
        return Result.error(
          AppError.validation(
      ...
      final record = await pb.collection(_programsCollection).getOne(id);
      final program = WorkoutPlan.fromJson(record.toJson());
  ...
  /// Validate workout plan data before creation or update
  Result<void> _validateWorkoutPlan(WorkoutPlan program) {
    ...
    if (program.description.length > 500) {
      return Result.error(
        AppError.validation(
          message: 'Program description cannot exceed 500 characters',
